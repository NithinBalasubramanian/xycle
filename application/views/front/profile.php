...
<?php
    ...
    <div class="post" style="height:auto;">
        <div class="plan">
            <div class="chi_plan1">
        <?php $profile_plan=$this->Admin_model->table_column('user','user_id',$data);
        if(count($profile_plan) > 0)
        {
            foreach($profile_plan as $row_plan)
            { 
                $profile_plan1=$this->Admin_model->table_column('plan','id',$row_plan['plan_type']);
                if(count($profile_plan1) > 0)
                {
                    foreach($profile_plan1 as $row_plan1)
                    { ?>
                            <p style="font-size:16px;font-weight:400;"><?php echo $row_plan1['plan']; ?></p>
                <?php }
                }
                }
        } ?>
        ...
        <div class="chi_plan2">
            <a href="<?php echo base_url(); ?>View/front/list_plan" class="btn btn-sm btn-success ">Upgrade Policy</a>
        </div>
        </div>
    </div>
    <div class="post" style="height:auto;">
