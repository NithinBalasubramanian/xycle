...
$pro_data=$this->Admin_model->table_column('groups','id',$data);
 ...
 <div class="first">
        <div class="top_part">
            <div class="post_profile">
                <div class="arrow">
                    <a href="<?php echo base_url(); ?>View/front/group_chat"><i class="fa fa-arrow-left arr_left" aria-hidden="true"></i></a>
                </div>
                <div class="post_profile_img1">
                <?php if($row1['group_img'] != ''){ ?>
                    <img src="<?php echo base_url();?>assets/user/group_img/image/<?php echo $row1['group_img']; ?>" alt="img" >
                <?php }else{ ?>
                    <img src="<?php echo base_url();?>assets/user/profile/default.png" alt="img" >
                <?php } ?>
                </div>
                <div class="post_profile_name1">
                    <div class="name" style="text-transform:uppercase;"><?php echo $row1['group_name']; ?>
                </div>
            </div>
...
<div class="third">
            ...
            <form method="post" id="chat_submit">
             <div class="fileDiv"><i class="fa fa-paperclip icon_21" aria-hidden="true"></i>
                ...
                <input type="hidden" name="group_to" value="<?php echo $data; ?>">
...
<script>
     $(document).on('submit','#chat_submit',function(e){
		...
		var url= "<?php echo base_url(); ?>Front/group_chat";
        $.ajax({
            type:'POST',
            url: url,
            data:formData,
    ...
    function repeat()
    {
        var group_id = "<?php echo $data; ?>";
        var receiver_id = "<?php echo $user_id; ?>";
        var base_url = "<?php echo base_url(); ?>";
        var tablename = 'group_chat';
		$.ajax({
            url: base_url+'Front/load_group_chat',
            type: 'POST',
            data: "group_id=" + group_id +"&receiver_id="+receiver_id+"&tablename="+tablename ,
            success: function(data) {
                $('#chat_load').html(data);
            }
        });
